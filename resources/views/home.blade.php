                        ...
                        <div class="accordion" id="accordionCountry">
                            ...
                            <div class="accordion-item">
                                <h2 class="accordion-header" id="heading{{ $country->id }}">
                                    <button class="accordion-button {{ $loop->first ? Null : 'collapsed' }}" type="button" data-bs-toggle="collapse" data-bs-target="#collapse{{ $country->id }}" aria-expanded="true" aria-controls="collapse{{ $country->id }}">
                                        {{ $country->name }}
                                    </button>
                                </h2>
                                <div id="collapse{{ $country->id }}" class="accordion-collapse collapse {{ $loop->first ? 'show' : Null }}" aria-labelledby="heading{{ $country->id }}" data-bs-parent="#accordionCountry">
                                    <div class="accordion-body row">
                                        @foreach($country->states as $state)
                                            <div class="col-lg-3 col-md-4 col-sm-6 col-xs-12">
                                                {{ $state->name }} ({{ $state->cities()->count() }})
                                            </div>
                                        @endforeach
                                    </div>
                                </div>
